...
class Dividend( models.Model):
    ...
    @staticmethod
    def format_inst_year_key(portfolio_id, instrument_id, year):
        return f"{portfolio_id}-{instrument_id}-{year}"

    def get_inst_year(self):
        return Dividend.format_inst_year_key(self.portfolio_id, self.instrument_id, self.payment_date.year)
...
class DividendRepository:
    ...
    def get_dividends_by_inst_year(self):
        self.divs_qs = (Dividend.objects.annotate(year=ExtractYear('payment_date'))
                                          .values('portfolio_id','instrument_id', 'year')
                        .annotate(total_amount=Sum('amount')))
        # convert to dict
        self.div_year_lookup = {Dividend.format_inst_year_key(div['portfolio_id'], div['instrument_id'], div['year'])
                                : div['total_amount'] for div in self.divs_qs}
        return self.div_year_lookup

    def get_dividend_total(self, portfolio_id, instrument_id, year_code="YTD"):
        year = datetime.now().year
        if year_code == 'LAST':
            year = year - 1
        elif year_code == 'PREV':
            year = year - 2
        key = Dividend.format_inst_year_key(portfolio_id, instrument_id, year)
